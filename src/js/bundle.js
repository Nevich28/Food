...
function cards() {
  ...
  class MenuItem {
    constructor(title, descr, price, imgUrl, alt) {
      let parentSelector = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : '.menu .container';
      this.title = title;
      this.descr = descr;
      this.price = price;
      this.imgUrl = imgUrl, this.alt = alt;
...
function forms(formSelector, modalTimerId) {
  ...
  const message = {
    //список сообщений для статусов загрузки
    loading: 'img/form/spinner.svg',
    success: 'Спасибо! Скоро мы с вами свяжемся',
    failure: 'Что-то пошло не так...'
  };
  forms.forEach(item => {
    //цикл для двух видов одной и той же формы
    bindPostData(item);
  });
  function bindPostData(form) {
    //при нажатии подтверждения в форме
    form.addEventListener('submit', e => {
      ...
      .then(data => {
        //сообщение о том что все нормально
        console.log(data);
        ...
        // очистка формы ввода в любом случае
        form.reset();
      });
    });
  }
  function showThanksModal(message) {
    //создание модалки после отправки запроса
...
function modal(triggerSalactor, modalSelector, modalTimerId) {
  // модальное окно
  const modalTrigger = document.querySelectorAll(triggerSalactor),
    // переменные
    modal = document.querySelector(modalSelector);
  modalTrigger.forEach(btn => {
    //запуск по нажатию на одну из кнопок
  ...
  function showModalByScroll() {
    // открытие при долистывании до низа, при этом если оно уже 1 раз показалось, то больше не будет
...
function slider(_ref) {
  ...
  let slideIndex = 1,
    offset = 0;
  if (slides.length < 10) {
    // добавляем вначале 0, если количество меньше 10
  ...
  function replacePx(instr) {
    //убираем рх в конце строки
  ...
  slides.forEach(slide => {
    //ставим всем слайдам одинаковую ширину
    slide.style.width = width;
  });
  slider.style.position = 'relative';
  const indicators = document.createElement('ol'),
    //делаем внизу индикатор прокрутки через список
    dots = [];
  indicators.classList.add('carousel-indicators');
  slider.append(indicators);
  for (let i = 0; i < slides.length; i++) {
    // добавляем индикатор прокрутки в верстку
  ...
  dots.forEach(dot => {
    //делаем кликабельным индикатор слайдера
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  function hideTabContent() {
    //тут мы скрываем табы и пункт меню
  ...
  function showContent() {
    ...
    //тут мы показываем табы и пункт меню
  ...
  tabsParent.addEventListener('click', event => {
    // при нажатии на нужный таб показываем его
...
function timer(id, deadline) {
  ...
  function getTimeRemaining(endtime) {
    // определение сколько времени до окончанмя таймера
  ...
  function getZero(num) {
    //добавка нуля перед числом если оно меньше 10
    if (num >= 0 && num < 10) {
      return `0${num}`;
    } else {
      return num;
  ...
  function setClock(selector, endtime) {
    ...
    function updateClock() {
      // непосредственное вбитие данных таймера в html
      ...
      if (t.total <= 0) {
        //остановка отсчета таймера если время вышло
...
const postData = async (url, data) => {
  //функция для отправки запроса на вервер и получения обратно ответа в json
